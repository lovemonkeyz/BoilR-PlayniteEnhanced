...
impl App for MyEguiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut Frame) {
        ...
        // TopBottomPanel must be declared BEFORE CentralPanel — CentralPanel fills
        // all remaining space and must always be last in each frame.
        if self.selected_menu == Menues::Settings {
            egui::TopBottomPanel::new(egui::panel::TopBottomSide::Bottom, "Bottom Panel")
                ...
                .show(ctx, |ui| {
                    ...
                    // texture_size() is None until the image is loaded into GPU memory;
                    // fall back to a sensible fixed size so the button is always visible.
                    let size = image.texture_size().unwrap_or(Vec2::new(32.0, 32.0));
